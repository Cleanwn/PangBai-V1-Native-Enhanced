...
namespace MdayS
{
	...
	std::optional<int32_t> ScriptPatches::Patch::GetPC()
	{
		...
		{
			g_logger.insert(log_levels::RED_ERROR, "Failed to find pattern for script ", m_Hash);
		}
		else
		{
	...
	std::shared_ptr<ScriptPatches::Patch> ScriptPatches::AddPatchImpl(joaat_t script, const std::string& pattern, int32_t offset, std::vector<uint8_t> patch)
	{
		...
		for (int i = 0; i < 176; i++)
		{
			if (g_patterns.m_script_programs[i] && g_patterns.m_script_programs[i]->m_NameHash == script)
	...
	void ScriptPatches::OnScriptVMEnterImpl(rage::scrProgram* program)
	{
		...
		{
			g_logger.insert(log_levels::RED_ERROR, "We've already shadowed the bytecode of another program!");
			return;
		}
