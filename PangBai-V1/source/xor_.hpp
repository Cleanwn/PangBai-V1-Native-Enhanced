...
class runtime_crypt {
public:
    runtime_crypt(std::string str, std::string key = "aAD8vsdfa")
        : m_string(str), m_key(key)
    {
        std::string encrypted_str;
        for (size_t i = 0; i < m_string.size(); ++i) {
            char c = m_string[i] ^ m_key[i % m_key.size()];
            encrypted_str += c;
        }
        m_string = encrypted_str;
    }
...
#define r_(str) runtime_crypt(str).decrypt()

#define x_(str) r_(xorstr_(str))
